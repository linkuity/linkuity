...
1. **Auto-match (>= 0.90).** `web-300` shares an exact **email**
   ...
   existing cluster.
2. **Review (>= 0.75, < 0.90).** `web-301` shares only the **surname token** `estrada`
   with `crm-300` — different email and phone — so it scores 0.80. The hub does **not**
   merge it; it opens a `review_threshold` task and leaves the record as its own cluster
...
| `web-301` | surname token only | 0.80 | review task |
