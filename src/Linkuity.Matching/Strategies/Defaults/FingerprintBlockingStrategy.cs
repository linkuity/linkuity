...
/// <summary>
/// Fingerprint blocking: one key per canonical-token variant (hyphen-joined forms collide:
/// WAL-MART ≡ WALMART) built from the canonical tokens — sorted, deduped, lowercased,
/// space-joined — so word-order, article, punctuation, and legal-suffix variants land in
/// the same block. Semantic-type-driven: a field participates only when its type has a
/// registered canonicalizer (currently OrganizationName). Emits "fp:{tokens}".
/// </summary>
