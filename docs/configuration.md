...
| `defaultPhoneRegion` | no | Two-letter uppercase ISO 3166-1 region code (`US`, `GB`, `AU`, …) used to read phone numbers written **without** a country code, which are otherwise ambiguous — the same digits are a valid subscriber number in many countries. Defaults to `US`. Numbers carrying an explicit country code ignore it. A number that is not valid in the configured region is left un-normalized rather than guessed at, so a wrong setting costs matches rather than silently relabelling numbers as another country's. Set this for any non-US feed of national numbers. Applies at ingest, on both the batch and durable paths. |
| `defaultDateOrder` | no | `MonthFirst` (default) or `DayFirst`. Decides how a slash-separated date whose first two components are ambiguous is read: `03/04/1980` is 3 April under `MonthFirst` and 4 March under `DayFirst`, and nothing in the value says which. **A wrong setting is quiet, not loud** — dates past the twelfth of a month fail to parse and pass through unchanged, while every earlier date parses to a confidently wrong value, so the field ends up partly raw and partly mislabelled with nothing to distinguish them. ISO forms (`yyyy-MM-dd`, `yyyy/MM/dd`) are unambiguous and read the same either way. An unrecognised value is rejected at load rather than defaulted. Set this for any feed not written US-style. Applies at ingest, on both the batch and durable paths. |
...
**Known limitation:** `identifierFloorGate` is one number per profile, so it can't
tell apart an identifier that's sufficient alone from one that only means
something with corroboration. `samples/durable/product-360/product.profile.json`
shows the problem: `gtin` (globally unique — a match really does mean the same
product) and `sku` (vendor-scoped only, so a lone match doesn't) both carry
weight `3.0` of `9.0` total, giving a bare-identifier-only match a weighted score
of `0.333` — just under the `0.35` default gate. In practice, a lone `gtin` match
with nothing else agreeing will not auto-merge on that profile. Lowering the gate
to admit it would simultaneously admit lone-`sku` merges, which are wrong, so the
gate is deliberately left as-is. Per-field identifier configuration (rather than
one gate per profile) is the real fix and is planned.
...
| `Identifier` | The field is a *strong identifier*: an exact match on it is decisive evidence — once the pair's weighted score also clears `identifierFloorGate`, the `identifier-weighted` scorer floors such a pair near auto-match — and it also produces an exact blocking key. Normally paired with `Matchable` and `Blocking`. |
