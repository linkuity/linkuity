...
The class that used to dominate this list — blocked, compared, and then left under the scoring threshold: Boeing, Walt Disney, Intel, 3M, Ford, Texas Instruments, Starbucks — is gone, closed by the canonical scoring change in Stage 2. Exactly one scoring miss survives it: `WALMART INC.` against the retired filer name `WAL MART STORES INC`, which scores **0.0** on name. Walmart still resolves correctly — its other records connect it transitively — but the pair itself is a genuine miss, and it is worth being precise about why, because the obvious diagnosis is wrong. The audit's decomposition states the position in one line: of 72 true pairs, 63 auto-merge, 8 are unreachable by blocking, and 1 is reachable but under-scored.

### The fix that measurement rejected

`WAL MART` versus `WALMART` looks like a word-splitting problem, and the obvious answer is to generate joined-token variants. It isn't, and they don't. The split half is *already* handled — `WAL MART INC` and `WALMART INC` both compress to `WALMART` and score 1.0 today, through the same compressed-form check that rescued Amazon. What actually holds the pair at zero is the extra token `STORES`, which exists on one side only and which token-set overlap penalizes as though it were contradictory evidence.

The mechanism that *would* close it is the one every commercial engine ships: treat generic business words — `STORES`, `GROUP`, `SYSTEMS`, `SERVICES` — as carrying little identity, and don't penalize a name for lacking one. Informatica calls them Company Skip Words, IBM tags and removes them, Splink discounts them by observed frequency. On this 49-company benchmark it worked perfectly: Walmart closed, and zero false pairs moved.

Then it was measured against all **1,052,432 company names in SEC's registrant file**, using CIK as held-out truth. Of the new matches it created, **95.6% joined companies with different CIKs.** Trimming the word list to the single best-evidenced entry, `STORES`, only improved that to 88%. Spot-checking the failures by hand explains why: `ALCO STORES INC` (a Texas retailer) and `ALCO, INC.` (an insurance company) collapse into one record, as do `AMERICAN STORES COMPANY` and `AMERICAN CORP`. The generic word was frequently the only thing distinguishing two real, unrelated registrants.

So the feature was not built. That is the same discipline as the scorecard above, pointed at a change rather than at a result: the benchmark said ship it, a corpus four orders of magnitude larger said the opposite, and the larger corpus was right. The honest resolution is to leave the pair unresolved and record why — and to note that the principled fix, weighting each token by how rare it actually is rather than deleting it from a list, fails *gracefully* where deletion fails catastrophically. `ALCO STORES` against `ALCO` would score weakly instead of perfectly. That is a different piece of engineering, and it is the one worth building next.
...
7. **Validate changes at a scale your benchmark cannot reach.** A curated 49-company benchmark is the right tool for measuring a pipeline and the wrong tool for clearing a rule that deletes information. The generic-word change above passed the benchmark perfectly and was 95.6% wrong across a million real company names, because 49 distinctive names contain no `ALCO STORES` versus `ALCO, INC.` A benchmark tells you whether a change helps the cases you thought of. Only volume tells you what it breaks.
